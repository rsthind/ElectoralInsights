...
public class ElectoralMap{
	//hasmap String of the county name
	//value is data class
	...
	public static void visualize(String state, int year) {
		try{
			...
			int xFactor = Math.abs((int)((maxLat - minLat)/(maxLong - minLong)));
			if(xFactor != 0) {
			...
			for(int i = 0; i < numSubregions; i++) {
				...
				if(votes == null) {
					...
					if(demVotes >= repubVotes){
						if(demVotes >= indepVotes) {
